...
int Read_Client(const OS_XML *xml, XML_NODE node, void *d1, __attribute__((unused)) void *d2)
{
    ...
    const char *xml_main_ip_update_time = "ip_update_time";
    ...
    for (i = 0; node[i]; i++) {
        ...
        } else if (strcmp(node[i]->element, xml_main_ip_update_time) == 0) {
            if (!OS_StrIsNum(node[i]->content)) {
                merror(XML_VALUEERR, node[i]->element, node[i]->content);
                return (OS_INVALID);
            }
            logr->main_ip_update_time = atoi(node[i]->content);
            if (logr->main_ip_update_time < 0) {
                merror(XML_VALUEERR, node[i]->element, node[i]->content);
                return (OS_INVALID);
            }
